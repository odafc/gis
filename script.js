...
function initMap(initPos) {
    // #map_containerにOSMタイルマップをレンダリング
    map = L.map("map_container").setView(
	...
	L.control.opacityLayers(baseCtl, overCtl, {collapsed:true}).addTo(map);

  fetch('https://odafc.github.io/ar/test.geojson', {method: 'GET'}).then(response => response.text()).then(text => {
  var data = JSON.parse(text);
  var frontera = L.geoJson(data, {
    style: {color: '#00f', weight: 3, opacity: 0.6, fillOpacity: 0.1, fillColor: '#00f'},
